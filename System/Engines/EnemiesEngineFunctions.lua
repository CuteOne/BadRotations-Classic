...
function updateOMEWT()
	...
	-- Add only units of concern to br.om
	for i = 1, #br.ewtOM do
		local thisUnit = br.ewtOM[i]
		if ObjectIsVisible(thisUnit) and (UnitReaction("player", thisUnit) <= 4 or GetUnitIsUnit(UnitCreator(thisUnit), "player"))
			and not isCritter(thisUnit) and getDistance(thisUnit) <= 50 and getLineOfSight(thisUnit)
		then
		...
		if not ObjectIsVisible(thisUnit) then
			br.ewtOM[i] = nil
			i = i - 1
		end
	end
